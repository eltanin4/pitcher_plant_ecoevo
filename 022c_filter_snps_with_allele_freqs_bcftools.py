...
df = pd.read_csv( '/home/akshitg/lte/metagenomes_lte/freebayes_out/snps/bcf_tsvs/mg_'
                  + str( mg_num ) + '_combined_bcftools.tsv', sep='\t' )

# CHOOSE DIRECTORY NAME APPROPRIATELY HERE.
contigs_file = open( '/home/akshitg/main/lte/sags_lte/D19-2600' + ( 2 - len( str( g_num ) ) ) * '0' +  
                     str( g_num ) + '/full_contig_names.txt', 'r' )
contig_names_raw = contigs_file.readlines( )
names_contigs = [ e.split( '\n' )[ 0 ].split(':')[1] for e in contig_names_raw ]
contigs_file.close()

PVAL_THRESH = 0
tgenedf = df[ df[ '#CHROM' ].isin( names_contigs ) ]
filtdf = tgenedf[ np.logical_and( tgenedf[ 'QUAL' ] >= PVAL_THRESH, tgenedf[ 'DP' ] >= 10 ) ]
filtdf = filtdf[ filtdf[ 'REF' ].map( len ) == 1 ]
...
snp_df.to_csv( '/home/akshitg/lte/metagenomes_lte/freebayes_out/snps/all_alleles/bcf_processed/mg_' 
