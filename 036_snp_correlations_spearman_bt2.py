...
for this_mic in range( 1, 11 ):
    ...
    try:
        df = pd.read_csv( '/home/akshitg/lte/metagenomes_lte/freebayes_out/snps/bt2/all_alleles/fb_trajs/mic_'
    ...
    if len( valid_snps[ this_mic - 1 ][ g_num ] ) > NUM_VALID_SNPS:
        ...
        for snp1, snp2 in combinations( range( len( valid_snps[ this_mic - 1 ][ g_num ] ) ), 2 ):
            ...
            pval_matrix[ snp1, snp2 ] = spearmanr( traj1, traj2 )[ 1 ]
            pval_matrix[ snp2, snp1 ] = spearmanr( traj2, traj1 )[ 1 ]
            this_matrix[ snp1, snp2 ] = spearmanr( traj1, traj2 )[ 0 ]
            this_matrix[ snp2, snp1 ] = spearmanr( traj2, traj1 )[ 0 ]

        for tsnp in range( len( valid_snps[ this_mic - 1 ][ g_num ] ) ):
            ttraj = valid_snps[ this_mic - 1 ][ g_num ][ tsnp ]
            this_matrix[ tsnp, tsnp ] = spearmanr( ttraj, ttraj )[ 0 ]
            pval_matrix[ tsnp, tsnp ] = spearmanr( ttraj, ttraj )[ 1 ]
        ...
        # Saving results.
        np.savetxt( '/home/akshitg/lte/metagenomes_lte/freebayes_out/snps/bt2/all_alleles/fb_spearman_corrs/valid_snps_mic_' 
                    + str( this_mic ) + '_gnm_' + str( g_num ) + '.txt', 
                    valid_snps[ this_mic - 1 ][ g_num ] )
        np.savetxt( '/home/akshitg/lte/metagenomes_lte/freebayes_out/snps/bt2/all_alleles/fb_spearman_corrs/corr_matrix_mic_' 
                    + str( this_mic ) + '_gnm_' + str( g_num ) + '.txt', 
                    this_matrix )
        np.savetxt( '/home/akshitg/lte/metagenomes_lte/freebayes_out/snps/bt2/all_alleles/fb_spearman_corrs/sign_matrix_mic_' 
                    + str( this_mic ) + '_gnm_' + str( g_num ) + '.txt', 
                    pval_matrix )
